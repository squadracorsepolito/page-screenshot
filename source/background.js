...
chrome.action.onClicked.addListener((tab) => {
	...
	chrome.tabs
		.sendMessage(tab.id, {action: 'take-screenshot'})
		.catch((error) => {
			// Handle connection error
			if (error.message?.includes('Receiving end does not exist')) {
				console.warn(
					'Page Screenshot: Content script not loaded yet. Please refresh the page and try again.',
				);
			} else {
